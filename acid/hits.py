...
from overview import get_potential, get_flux
...
if False:
    ...
    np.savez('hits.npz', hits=hits, pots=pots, fluxes=fluxes)
else:
    dct = np.load('hits.npz')
    fluxes = dct['fluxes']
    pots = dct['pots']
    hits = dct['hits']
...
for iflux in range(15):
    ...
    plt.text(.6, offset+5, '%.1e' % fluxes[iflux], ha='right')
